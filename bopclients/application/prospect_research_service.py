...
class ProspectResearchService:
    ...
    def research_prospect(
        ...
        """Research a single prospect for an organization."""
    ...
    def refresh_research(
        ...
        return self.research_prospect(org_id, prospect_id, campaign_id)

    def research_campaign(
        self, org_id: str, campaign_id: str, limit: int = 100
    ) -> Dict[str, Any]:
        ...
        for p in selected:
            try:
                res = self.orchestrator.research_prospect(org_id, p.id, campaign_id)
                results.append(res)
